...
const (
	maxManualBTCETHLeverage = store.MaxLeverageLimit // Allow up to exchange maximum (125x)
	maxManualAltLeverage    = store.MaxLeverageLimit
)
...
func validateTraderLeverageRange(btcEthLeverage, altcoinLeverage int) (string, string) {
	if btcEthLeverage < 0 || btcEthLeverage > maxManualBTCETHLeverage {
		return traderCreationRequestError(fmt.Sprintf("BTC/ETH leverage must be between 1x and %dx", maxManualBTCETHLeverage)), "trader.create.invalid_btc_eth_leverage"
	}
	if altcoinLeverage < 0 || altcoinLeverage > maxManualAltLeverage {
		return traderCreationRequestError(fmt.Sprintf("Altcoin leverage must be between 1x and %dx", maxManualAltLeverage)), "trader.create.invalid_altcoin_leverage"
	}
	return "", ""
}
...
func missingExchangeFields(exchange *store.Exchange) []string {
	...
	case "simulated":
		// Simulated trading doesn't require API keys
	}
...
func validateExchangeForTraderCreation(exchange *store.Exchange) (string, string, map[string]string) {
	...
	switch exchange.ExchangeType {
	case "binance", "bybit", "okx", "bitget", "gate", "kucoin", "hyperliquid", "aster", "lighter", "indodax", "simulated":
		return "", "", nil
	default:
		return formatTraderCreationError(
