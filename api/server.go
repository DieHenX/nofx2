...
Use this to enable/disable an exchange or update API credentials. The "id" field is required to identify which exchange to update.`,
			...
			s.routeWithSchema(protected, "GET", "/notifications/config", "Get email notification configuration",
				`Returns: {"email_enabled":<bool>,"email_host":"<string>","email_port":<int>,"email_user":"<string>","has_email_pass":<bool>,"email_from_name":"<string>","email_to":"<string>","notify_on_open":<bool>,"notify_on_close":<bool>,"notify_on_stop_loss":<bool>,"notify_on_take_profit":<bool>,"notify_on_error":<bool>,"notify_daily_summary":<bool>}
NOTE: Email password is never returned in the response; only has_email_pass indicates whether one is configured.`,
				s.handleGetNotificationConfig)
			s.routeWithSchema(protected, "PUT", "/notifications/config", "Update email notification configuration",
				`Body: {"email_enabled":<bool>,"email_host":"<string, default smtp.qq.com>","email_port":<int, default 465>,"email_user":"<string>","email_pass":"<string, leave empty to keep current>","email_from_name":"<string, default NOFX Trading>","email_to":"<string>","notify_on_open":<bool>,"notify_on_close":<bool>,"notify_on_stop_loss":<bool>,"notify_on_take_profit":<bool>,"notify_on_error":<bool>,"notify_daily_summary":<bool>}
Leave email_pass empty to preserve the existing password. Supports encrypted transmission.`,
				s.handleUpdateNotificationConfig)
			s.routeWithSchema(protected, "POST", "/notifications/test-email", "Send a test email to verify notification configuration",
				`Body: {"to_email":"<string, optional — defaults to configured email_to>"}
The email notification settings must be configured first.`,
				s.handleTestEmail)
...
StrategyConfig fields:
  ...
  risk_control.btc_eth_max_leverage: BTC/ETH max leverage (1-125, user configurable, 0=use exchange max)
  risk_control.altcoin_max_leverage: altcoin max leverage (1-125, user configurable, 0=use exchange max)
  ...
  risk_control.min_position_size: minimum USDT per trade (0=disabled, user configurable)
