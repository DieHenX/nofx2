...
func (tm *TraderManager) addTraderFromStore(traderCfg *store.Trader, aiModelCfg *store.AIModel, exchangeCfg *store.Exchange, st *store.Store) error {
	...
	case "simulated":
		// Simulated trading doesn't require API keys; uses market data from Binance by default
		traderConfig.SimulatedExchange = "binance"
	}
...
// RefreshAllTradersNotificationConfig refreshes the notification config for all loaded traders
func (tm *TraderManager) RefreshAllTradersNotificationConfig() {
	tm.mu.RLock()
	defer tm.mu.RUnlock()

	for _, t := range tm.traders {
		t.RefreshNotificationConfig()
	}
	logger.Infof("🔔 Refreshed notification config for %d traders", len(tm.traders))
}
