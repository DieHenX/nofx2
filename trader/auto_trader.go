...
type AutoTraderConfig struct {
	...
	SimulatedExchange string // For simulated trading, which exchange to get market data from (binance/gate/etc)
...
type AutoTrader struct {
	...
	notifier              *notify.Manager    // Notification manager
...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	case "simulated":
		logger.Infof("🎮 [%s] Using Simulated trading (Paper Trading)", config.Name)
		exchangeForMarket := "binance" // Default to Binance for market data
		if config.SimulatedExchange != "" {
			exchangeForMarket = config.SimulatedExchange
		}
		if config.InitialBalance <= 0 {
			config.InitialBalance = 10000 // Default 10000 USDT
		}
		trader = simulated.NewSimulatedTrader(config.InitialBalance, exchangeForMarket)
		logger.Infof("🎮 [%s] Simulated trader initialized with %.2f USDT (market data from %s)", config.Name, config.InitialBalance, exchangeForMarket)
	default:
	...
	at.refreshNotificationConfig()

	return at, nil
}

// RefreshNotificationConfig refreshes the notification config from the database (public wrapper)
func (at *AutoTrader) RefreshNotificationConfig() {
	at.refreshNotificationConfig()
}
...
func (at *AutoTrader) refreshNotificationConfig() {
	...
	// Try to get specific config for this trader
	config, err := at.store.NotificationConfig().GetByTraderID(at.userID, at.id)
	if err != nil || config == nil || !config.EmailEnabled {
		// Fallback to global config
		config, err = at.store.NotificationConfig().GetGlobal(at.userID)
		if err != nil || config == nil {
			return
		}
	}

	if config.EmailEnabled && config.EmailUser != "" && string(config.EmailPass) != "" {
		if at.notifier == nil {
			at.notifier = notify.NewManager()
		}
		at.notifier.SetEmailNotifier(&notify.EmailConfig{
		...
		logger.Infof("🔔 [%s] Email notifications enabled (%s)", at.name, config.EmailUser)
	} else {
		at.notifier.SetEmailNotifier(nil)
	}
}
...
func (at *AutoTrader) Run() error {
	...
	// Start Simulated trader stop check if using Simulated exchange
	if at.exchange == "simulated" {
		if simTrader, ok := at.trader.(*simulated.SimulatedTrader); ok {
			// Set notification callback
			simTrader.OnStopTriggered = func(symbol string, side string, qty float64, entry float64, exit float64, pnl float64, stopType string) {
				if at.notifier != nil {
					if stopType == "SL" {
						go at.notifier.SendStopLossNotification(at.name, at.exchange, symbol, qty, entry, exit, exit)
					} else {
						go at.notifier.SendTakeProfitNotification(at.name, at.exchange, symbol, qty, entry, exit, exit, pnl)
					}
				}
			}
			...
			go func() {
				...
				for {
					select {
					case <-ticker.C:
						simTrader.CheckAndExecuteStops()
					case <-at.stopMonitorCh:
						return
					}
				}
			}()
			at.logInfof("🔄 Simulated stop-loss/take-profit check enabled (every 5s)")
		}
	}

	// Start daily summary scheduler
	at.startDailySummaryScheduler()
...
// startDailySummaryScheduler starts a goroutine that sends a daily trading summary
func (at *AutoTrader) startDailySummaryScheduler() {
	at.monitorWg.Add(1)
	go func() {
		defer at.monitorWg.Done()
		...
		at.logInfof("📅 Daily summary scheduler set for first run at %s (in %.0f minutes)", next.Format("15:04"), initialDelay.Minutes())
		...
		for {
			...
			case <-ticker.C:
				at.sendDailySummary()
			case <-at.stopMonitorCh:
				at.logInfof("📅 Daily summary scheduler stopped")
				return
			}
		}
	}()
...
// sendDailySummary sends the daily trading summary notification
func (at *AutoTrader) sendDailySummary() {
	if at.notifier == nil {
		return
	}
	...
	if err != nil {
		at.logWarnf("📅 Failed to get trading stats for daily summary: %v", err)
		// Send summary with zero stats
	...
	go at.notifier.SendDailySummary(at.name, at.exchange, stats.TotalTrades, stats.WinTrades, stats.TotalPnL, openPositions)
	at.logInfof("📅 Daily summary sent: %d trades, %.2f PnL, %d open positions", stats.TotalTrades, stats.TotalPnL, openPositions)
}
