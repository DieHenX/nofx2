...
func (at *AutoTrader) runCycle() error {
	...
	if err != nil {
		...
		// Send error notification on AI failure (first failure only, avoid spam)
		if at.consecutiveAIFailures == 1 {
			if at.notifier != nil {
				go at.notifier.SendErrorNotification(at.name, at.exchange, fmt.Sprintf("AI call failed: %v", err))
			}
		}
		...
		if at.consecutiveAIFailures >= 3 && !at.isSafeMode() {
			...
			// Send error notification for safe mode activation
			if at.notifier != nil {
				go at.notifier.SendErrorNotification(at.name, at.exchange, fmt.Sprintf("SAFE MODE ACTIVATED: AI failed %d consecutive times. No new positions will be opened.", at.consecutiveAIFailures))
			}
		}
	...
	for _, d := range sortedDecisions {
		...
		if err := at.executeDecisionWithRecord(&d, &actionRecord); err != nil {
			...
			// Send error notification for execution failure
			if at.notifier != nil {
				go at.notifier.SendErrorNotification(at.name, at.exchange, fmt.Sprintf("Order execution failed: %s %s - %v", d.Symbol, d.Action, err))
			}
		} else {
