...
func (at *AutoTrader) applyAutopilotFullSizeOpen(decision *kernel.Decision, equity float64) {
	...
	// No hardcoded upper limit - use user's configured value
	if leverage > store.MaxLeverageLimit {
		leverage = store.MaxLeverageLimit
	}
...
// When MinPositionSize is set to 0, the minimum position size check is disabled
func (at *AutoTrader) enforceMinPositionSize(positionSizeUSD float64) error {
	...
	// When MinPositionSize is 0, the minimum check is disabled
	if minSize <= 0 {
		return nil
	}
