...
func (s *Store) initTables() error {
	...
	if err := s.NotificationConfig().initTables(); err != nil {
		return fmt.Errorf("failed to initialize notification config tables: %w", err)
	}
	return nil
}
...
// NotificationConfig gets notification config storage
func (s *Store) NotificationConfig() *NotificationConfigStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.notificationConfig == nil {
		s.notificationConfig = NewNotificationConfigStore(s.gdb)
	}
	return s.notificationConfig
}
