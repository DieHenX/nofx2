...
// NOTE: MinPositionSize and leverage limits can now be customized by users
// The following constants are only used for extreme bounds checking
const (
	...
	MinConfidence     = 50
	MaxConfidence     = 100
	MinPositionRatio  = 0.1  // Minimum position value ratio (allow more flexible settings)
	// MinPositionSize is now user-configurable with no enforced minimum
	// MaxPositionSize is the absolute upper bound for safety
	MinPositionSizeLimit = 0.0   // User can set 0 to disable minimum
	MaxPositionSizeLimit = 10000 // Absolute maximum for any safety limit
	// Leverage limits are now user-configurable
	// The exchange maximum will be used as the upper bound when user sets 0
	MaxLeverageLimit = 125 // Absolute maximum (some exchanges allow up to 125x)
)
...
func (c *StrategyConfig) ClampLimits() {
	...
	// Clamp leverage limits - now fully user-configurable
	// 0 means use exchange maximum, so we only enforce minimum of 1
	if c.RiskControl.BTCETHMaxLeverage < MinLeverage {
		c.RiskControl.BTCETHMaxLeverage = MinLeverage
	}
	if c.RiskControl.BTCETHMaxLeverage > MaxLeverageLimit {
		c.RiskControl.BTCETHMaxLeverage = MaxLeverageLimit
	}
	if c.RiskControl.AltcoinMaxLeverage < MinLeverage {
		c.RiskControl.AltcoinMaxLeverage = MinLeverage
	}
	if c.RiskControl.AltcoinMaxLeverage > MaxLeverageLimit {
		c.RiskControl.AltcoinMaxLeverage = MaxLeverageLimit
	}
	...
	// MinPositionSize: 0 means disabled (no minimum), user can set any value
	if c.RiskControl.MinPositionSize < MinPositionSizeLimit {
		c.RiskControl.MinPositionSize = MinPositionSizeLimit
	}
	if c.RiskControl.MinPositionSize > MaxPositionSizeLimit {
		c.RiskControl.MinPositionSize = MaxPositionSizeLimit
	}
...
func GetDefaultStrategyConfig(lang string) StrategyConfig {
	...
	config := StrategyConfig{
		...
		RiskControl: RiskControlConfig{
			...
			BTCETHMaxLeverage:            20,  // BTC/ETH max leverage (user configurable, 1-125)
			AltcoinMaxLeverage:           20,  // Altcoin max leverage (user configurable, 1-125)
			...
			MinPositionSize:              0,   // Min position size (0=disabled, user configurable)
